...
exports.expandComponentGraphs = expandComponentGraphs;
 ...
 */
function expandComponentGraphs(flowNodes, flowEdges, schemaNodes = [], schemaEdges = []) {
    const resultNodes = [];
    const resultEdges = [...flowEdges];
    ...
    flowNodes.forEach(node => {
        ...
        let pkgResolved = (0, packageResolver_1.resolvePackageImplementation)(packageId);
        if ((!pkgResolved.graph && !pkgResolved.subflow) || pkgResolved.packageId === 'unknown') {
            ...
            if (label.toLowerCase().includes('ultrasonic')) {
                pkgResolved = (0, packageResolver_1.resolvePackageImplementation)('ultrasonic_hcsr04');
            }
        }
        ...
        const pkgDef = (0, components_1.getComponentPackage)(pkgResolved.packageId);
        ...
        const internalNodes = internalGraph.nodes;
        const internalEdges = internalGraph.edges;
        const startNode = internalNodes.find(n => n.data?.nodeType === 'start' || n.type === 'start' || n.id === 'start');
        ...
        const clonedSubNodes = [];
        internalNodes.forEach(subNode => {
            const subNodeType = subNode.data?.nodeType || subNode.type;
        ...
        internalEdges.forEach(subEdge => {
            if (subEdge.source === startNode?.id)
                return;
            resultEdges.push({
                ...subEdge,
        ...
        const returnNode = internalNodes.find(n => n.data?.nodeType === 'return');
...
function subNodeIdIsExit(node, edges) {
