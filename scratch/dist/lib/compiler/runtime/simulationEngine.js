...
class SimulationEngine {
    ...
    loadProgram(program, schemaNodes) {
        ...
        schemaNodes.forEach(node => {
            if (node.id !== 'arduino-uno') {
                ...
                const pluginType = (0, plugin_1.mapLabelToPluginType)(label) || node.data?.componentType;
        ...
        const loopStatements = program.body.filter((stmt) => stmt.kind !== 'FunctionDeclaration');
        this.callStack.push({
            functionName: 'loop',
            variables: {},
    ...
    setHardwareMockValue(safeName, key, value) {
        ...
        try {
            switch (stmt.kind) {
                ...
                case 'ForLoop': {
                    ...
                    const loopVarName = stmt.init.name;
                    const startVal = this.evaluate(stmt.init.value, frame.variables);
                ...
                case 'ExpressionStatement': {
                    ...
                    if (expr.kind === 'CallExpression' && expr.callee.includes('.custom')) {
                        ...
                        const plugin = plugin_1.pluginRegistry.get(pluginType);
                        const rawParams = expr.arguments[0];
                        let params = {};
                        ...
                        const evalPrintText = (rawStr, vars) => {
                            const parts = this.parsePrintArguments(rawStr);
                            return parts.map(part => {
                                const parsedExpr = (0, expressionParser_1.parseExpressionString)(part);
                                try {
                        ...
                        if (pluginType === 'dht') {
                            ...
                            const defaultVars = { ldr: 'lightVal', soilMoisture: 'moisture', waterLevel: 'waterLevel', mqGas: 'gasVal' };
                            ...
                            const defaultVars = { pir: 'motion', ir: 'obstacle', vibration: 'vibration', flame: 'flameVal' };
                            const varName = params[defaultVars[pluginType]] || defaultVars[pluginType];
    ...
    pushFrame(name, statements, variables) {
