...
class SemanticAnalyzer {
    ...
    validateStatement(stmt, scope) {
        ...
        switch (stmt.kind) {
            case 'BlockStatement': {
                const blockScope = new symbolTable_1.SymbolTable(scope, scope.scopeType);
                stmt.body.forEach(s => this.validateStatement(s, blockScope));
                break;
            }
            case 'FunctionDeclaration': {
                const funcScope = new symbolTable_1.SymbolTable(scope, 'function');
            ...
            case 'IfStatement':
                ...
                break;
            case 'ForLoop': {
                const loopScope = new symbolTable_1.SymbolTable(scope, 'loop');
