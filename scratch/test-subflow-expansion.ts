...
function runSubflowExpansionTests() {
  ...
  const expansion = expandComponentGraphs(userFlowNodes, userFlowEdges, schemaNodes, schemaEdges);
  ...
  const gpioHighParams = (gpioHighNode?.data?.params as any) || {};
  ...
  const pulseInParams = (pulseInNode?.data?.params as any) || {};
  ...
  const compiler = new GraphToASTCompiler(userFlowNodes, userFlowEdges);
