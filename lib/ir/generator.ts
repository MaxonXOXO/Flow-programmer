...
export class ArduinoUnoGenerator {
  ...
  public generate(
    ...
    this.connections.forEach(conn => {
      ...
      seenComponents.has(conn.componentId);
      seenComponents.add(conn.componentId);

      // Get all connections for this component to match pin mapping, ignoring power/ground
      ...
      // Find component parameters in schema nodes
    ...
    program.body.forEach(stmt => {
      if (stmt.type === 'ExpressionStatement' && stmt.expression.type === 'LiteralExpression') {
        ...
        if (valStr.startsWith('{"type":"FunctionDeclaration"')) {
          // Parse stringified function declaration
          try {
            const funcDecl = JSON.parse(valStr) as FunctionDeclaration;
    ...
    const mainBlock: BlockStatement = {
      type: 'BlockStatement',
      body: mainBody
    };
    ...
    const sections: string[] = [
      `/*`,
      ` * Generated by Flow Programmer (Universal IR compiler)`,
  ...
  private generateBlock(block: BlockStatement, indent: number, declaredVars: Set<string>): string {
  ...
  private generateStatement(stmt: Statement, indent: number, declaredVars: Set<string>): string {
    const pad = '  '.repeat(indent);
    ...
    switch (stmt.type) {
      ...
      case 'ForLoop': {
        ...
        const condCode = this.generateExpression(stmt.condition);
        const updCode = stmt.update.type === 'AssignmentStatement'
          ? this.generateStatement(stmt.update, 0, initScope).trim().slice(0, -1)
          : this.generateExpression(stmt.update as Expression);
      ...
      case 'ExpressionStatement': {
        // Intercept hardware plugin method mappings
        if (stmt.expression.type === 'FunctionCallExpression' && stmt.expression.callee.endsWith('.custom')) {
          const pluginType = stmt.expression.callee.split('.')[0];
          const plugin = pluginRegistry.get(pluginType);
          if (plugin && plugin.codegen.customCodegen) {
            // Retrieve instance name from connections
            ...
            // Pack connections pins Map, ignoring power/ground
            const pinsMap: Record<string, string> = {};
            this.connections.filter(c => c.componentId === conn?.componentId && !this.isPowerPin(c)).forEach(c => {
              pinsMap[c.pin] = this.pinToNumber(c.arduinoPin);
            });

            // Retrieve packed params literal
            let params: Record<string, string> = {};
            const paramLit = stmt.expression.arguments[0] as LiteralExpression;
            if (paramLit && paramLit.type === 'LiteralExpression') {
              try {
...
export type CodeGenerator = ArduinoUnoGenerator;
