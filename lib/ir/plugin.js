...
exports.mapLabelToPluginType = mapLabelToPluginType;
exports.formatStringLiteral = formatStringLiteral;
exports.parsePrintArguments = parsePrintArguments;
function mapLabelToPluginType(label) {
    const lbl = label.toLowerCase();
    if (lbl.includes('dht'))
        return 'dht';
    if (lbl.includes('ultrasonic'))
        return 'ultrasonic';
    if (lbl.includes('servo'))
        return 'servo';
    if (lbl.includes('lcd'))
        return 'lcd';
    if (lbl.includes('oled'))
        return 'oled';
    if (lbl.includes('ldr'))
        return 'ldr';
    if (lbl.includes('soil'))
        return 'soilMoisture';
    if (lbl.includes('water'))
        return 'waterLevel';
    if (lbl.includes('gas') || lbl.includes('mq'))
        return 'mqGas';
    if (lbl.includes('pir'))
        return 'pir';
    if (lbl.includes('ir sensor') || lbl.includes('obstacle'))
        return 'ir';
    if (lbl.includes('vibration'))
        return 'vibration';
    if (lbl.includes('flame'))
        return 'flame';
    if (lbl.includes('l298n'))
        return 'l298n';
    if (lbl.includes('l293d'))
        return 'l293d';
    return undefined;
}
...
function parsePrintArguments(str) {
    if (!str)
        return [];
    const parts = [];
    let current = '';
    ...
    registerDefaultPlugins() {
        ...
        this.register({
            ...
            simulation: {
                ...
                execute: (methodName, args, params, simState) => {
                    if (methodName === 'readTemperature')
                        return { value: simState.temperature || 24.0, updatedState: simState };
                    if (methodName === 'readHumidity')
                        return { value: simState.humidity || 45.0, updatedState: simState };
        ...
        this.register({
            ...
            simulation: {
                ...
                execute: (methodName, args, params, simState) => {
                    if (methodName === 'readDistance')
                        return { value: simState.distance || 50, updatedState: simState };
                    return { value: 0, updatedState: simState };
                }
            }
        });
        ...
        const registerAnalogSensor = (type, label, defaultPin, defaultVar, icon) => {
            this.register({
                type,
                label,
                icon,
        ...
        const registerDigitalSensor = (type, label, defaultPin, defaultVar, icon) => {
            this.register({
                type,
                label,
                icon,
        ...
        const registerMotorDriver = (type, label) => {
            this.register({
                type,
                label,
                icon: 'Cpu',
